defmodule HrBrandAgentWeb.ResearchLive.Results do
            ...
            <div class="p-6">
              <%= if @funnel do %>
                <div class="space-y-6">
                  ...
                  <div>
                    <div class="flex justify_between items-center mb-2">
          ...
          <div class="mt-8 bg-white rounded-lg shadow">
            ...
            <div class="p-6">
              <div class="space-y-4">
                ...
                <% end %>
                <%= for rec <- (get_in(@funnel || %{}, [:job_descriptions, :recommendations]) || []) do %>
                  <div class="flex items-start space-x-3">
                    <.icon name="hero-light-bulb" class="w-5 h-5 text-yellow-500 mt-0.5" />
                    <p class="text-gray-700"><%= rec %></p>
                  </div>
  ...
  defp has_recommendations?(red_flags, funnel) do
    red_recs = red_flags[:recommendations] || []
    funnel_recs = get_in(funnel || %{}, [:job_descriptions, :recommendations]) || []
    length(red_recs) > 0 or length(funnel_recs) > 0
  end
